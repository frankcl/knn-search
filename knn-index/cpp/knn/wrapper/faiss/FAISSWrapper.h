...
private:
    ...
    static void configIndex(::faiss::Index* index, const std::unordered_map<std::string, std::string>& paramMap);
...
public:
    ...
    static void dump(const std::string& indexDescription, const FAISSData& indexData,
                     const std::unordered_map<std::string, std::string>& paramMap, const std::string& path);
};
